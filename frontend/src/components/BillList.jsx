...
export default function BillList() {
    ...
    const paginateBills = async () => {
      ...
      while (!done) {
        const res = await fetch(
          `${API_URL}/api/v1/bills/2023/search?offset=${offset}`
        );
  ...
  let filteredBills = bills
    .filter((x) => {
      const currentlyInCommittee =
        x.status?.committeeName &&
        x.status.committeeName.toLowerCase() === committee.toLowerCase();
      const wasInCommittee = () => {
        if (!x?.milestones?.items) return false;
        for (const status of x.milestones.items) {
          if (status.committeeName?.toLowerCase() === committee.toLowerCase())
            return true;
        }
        return false;
      };
      ...
      return (
        infilter &&
        inSearch &&
        (!committee || currentlyInCommittee || wasInCommittee())
      );
    })
  ...
  return (
    <div>
      ...
      <section className="content">
        ...
        <div className="table-head" onClick={filterByStatus('IN_SENATE_COMM')}>
          IN COMMITTEE
        </div>
        <div className="table-head" onClick={filterByStatus('SENATE_FLOOR')}>
          ON FLOOR CALENDAR
        </div>
        <div className="table-head pass-two">
          <p
            className="header-senate-paragraph"
            onClick={filterByStatus('PASSED_SENATE')}
          >
            PASSED SENATE
          </p>
          <p
            className="header-assembly-paragraph"
        ...
        <div className="table-head" onClick={filterByStatus('SIGNED_BY_GOV')}>
          SIGNED BY GOVERNOR
        </div>
      </section>
      <div className="bill">
